...
class BRocScaler {
// Container class of ROC scaler data in NormAna.  All data public, so
// it is like a "struct".
 public:
...
class BNormData {
// Container class used by NormAna
public:
  BNormData() {
    evcnt = new Int_t[5];
...
class NormAna : public THaPhysicsModule {
  
public:
   NormAna( const char* name="N", const char* description="Normalizion Analysis" ,const char* filename="scaler%d.log",const Int_t maxt=12);
   virtual ~NormAna();
...
private:
  ...
  ClassDef(NormAna,0)  // Normalization analysis utilities
};
