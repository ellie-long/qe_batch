...
class BRocScaler {
// Container class of ROC scaler data in NormAna_R.  All data public, so
// it is like a "struct".
 public:
...
class BNormData {
// Container class used by NormAna_R
public:
  BNormData() {
    evcnt = new Int_t[5];
...
class NormAna_R : public THaPhysicsModule {
  
public:
   NormAna_R( const char* name="N", const char* description="Normalizion Analysis" ,const char* filename="scaler%d.log",const Int_t maxt=12);
   virtual ~NormAna_R();
...
private:
  ...
  ClassDef(NormAna_R,0)  // Normalization analysis utilities
};
