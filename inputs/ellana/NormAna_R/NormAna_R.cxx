...
//      NormAna_R* norm = 
//        new NormAna_R("N","Normalization Analysis");
//      gHaPhysics->Add(norm);
// 
// 2. Add variables N.* to the output tree.
//
// 3. NormAna_R will produce an end-of-run printout.
//
...
NormAna_R::NormAna_R( const char* name, const char* descript ,const char* filename ,const Int_t maxt) :
...
//_____________________________________________________________________________
Int_t NormAna_R::SetupRawData( const TDatime* run_time, EMode mode )
{
  Int_t retval = 0;
...
//_____________________________________________________________________________
Int_t NormAna_R::End( THaRunBase* run ) 
{
...
//_____________________________________________________________________________
  void NormAna_R::WriteHist()
{
  for (UInt_t i = 0; i < hist.size(); i++) hist[i]->Write();
}

//_____________________________________________________________________________
  void NormAna_R::BookHist()
{
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus NormAna_R::Init( const TDatime& run_time ) 
{

  fStatus = kNotinit;
  MakePrefix();

  cout << "NormAna_R:: Init !"<<endl<<flush;
  ...
  while (THaApparatus *apps = static_cast<THaApparatus*>(next1())){
    if (CmpNoCase("B",apps->GetName()) == 0){
      myhelicity  = (THaADCHelicity*)(apps->GetDetector("adchel.R"));
      break;
    }
  }
  ...
  while( THaScalerGroup* tscalgrp = static_cast<THaScalerGroup*>( next() )) {
    ...
    if (CmpNoCase("Right",scaler->GetName()) == 0) {
...
//_____________________________________________________________________________
Int_t NormAna_R::Process(const THaEvData& evdata)
{
...
// if helicity was enabled.
  ...
  // cout << "NormAna_R:  process the data ..."<<endl<<flush;
...
//FIXME: temporary workaround for helicity work-in-progress
  ...
  Int_t targethel=0;
  Double_t data1 = static_cast<Double_t>(evdata.GetData( 2, 23, 13, 0 ));
  Double_t data2 = static_cast<Double_t>(evdata.GetData( 2, 23, 12, 0 ));
...
// (KA) pulled from $DB/20090429/db_D.dat
  int crate = 2;
  int slot = 12;
  int startchan = 64;
...
//_____________________________________________________________________________
void NormAna_R::Print( Option_t* opt ) const {
...
//_____________________________________________________________________________
void NormAna_R::SetEventInt(Int_t *event)
{
...
//_____________________________________________________________________________
Int_t NormAna_R::BcmCalib(const THaEvData& evdata) {
...
void NormAna_R::GetRocScalers(const THaEvData& evdata) {
...
void NormAna_R::CalcAsy( ) {
  // do something with norm_plus and norm_minus ...

}


void NormAna_R::TrigBits( Int_t helicity ) {
