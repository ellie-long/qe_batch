...
void OnlineGUI::TreeDraw(drawcommand command) {
  ...
  if (iTree <= fRootTree.size() ) {
    ...
    hobj = (TObject*)gROOT->FindObject("htemp");
    if(errcode==-1) {
  ...
  if(showGolden) {
    if(!hobj) return;
    fGoldenFile->cd();
    if (iTree <= fGoldenFile.size() ) {
      TObject *htemp_golden = hobj->Clone(); // FIXME memory leak here
      TH1* thathist = (TH1*)hobj;
      TH1* goldhist = (TH1*)htemp_golden;
      goldhist->SetName("htemp_golden");
      errcode = fGoldenFile[iTree]->Project("htemp_golden",var,cut);
      if(errcode==-1) {
        BadDraw(var+" not found");
      } else if (errcode!=0) {
...
void OnlineGUI::PrintPages() {
  ...
  if(!fRootFile->IsOpen()) {
    ...
    GetRootTree();
    GetTreeVars();
    for(UInt_t i=0; i<fGoldenFile.size(); i++) {
      if(fRootTree[i]==0) {
	fRootTree.erase(fRootTree.begin() + i);
      }
    }
