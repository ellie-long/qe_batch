...
#include "THaEvent.h"
#include "THaOutput.h"
// If there is a problem, uncomment THaEvData.h, THaScalerGroup.h, THaCodaData.h, and THaBenchmark.h
//#include "THaEvData.h"
...
void ReplayCore(
				...
				Bool_t QuietRun =kTRUE       //whether not ask question?
				...
				)
{
	...
	for (Int_t nsplit=0;!exit;nsplit++)
	{
		...
		if ( ((!path || !*path) && !found) || oldfilename==filename) {
			...
			cout << "Scidda marinky dinky dink, scidda mirinky doo: I love you" << endl; 
	...
	cout << "Deleting gHaApps..." << endl;
	gHaApps->Delete();
	cout << "Deleting gHaPhysics..." << endl;
	gHaPhysics->Delete();
	cout << "Deleting gHaScalers..." << endl;
	gHaScalers->Delete();
	cout << "Closing analyzer..." << endl;
	analyzer->Close();
