...
void test_hand_target_asymmetry_vs_nu(){
...
//		double TDCmax = 1500;
		...
		double ADCmin = -100;
		double ADCmax = 11100;
		double goodTDCleftcutmin = 0;
		double goodTDCleftcutmax = 3100;
		double goodTDCrightcutmin = 0;
...
//			HANDrightcut->Scale(1/(HeChargeScale*Helivetime));
			...
			if (includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && vetoTDCs && goodTDCcut && firstTrack && "he3R.Spin==1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && goodTDCcut && firstTrack && "he3R.Spin==1","");}
			...
			if (includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && vetoTDCs && goodTDCcut && firstTrack && "he3R.Spin==-1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && goodTDCcut && firstTrack && "he3R.Spin==-1","");}
