#define charge_analysis_class_cxx
#include "charge_analysis_class.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <time.h>
...
// **************** Unknown, Q2=??? *******************
	cout << "Processing Unknown, Q2=??? dataset..." << endl;
	dataset = "Uknown, Q2=???";
	int HeRunNumber = inRunNumber;
...
/*		if (isGoodForAllBasic)
		{
			...
			if (goodEvent)
			{
				...
				for (int k=0; k<11; k++) {hand_class_fill_good_events(ToFgoodEvent[k], ToFcut[k], ToFup[k], ToFdown[k], nuAllCuts[k], nuUp, nuDown, tofCutMin, tofCutMax, nuMin, nuMax, nuBins, asymType,k);}
...
/*			if (firstTime)
			{
				if (int(HeSpin)==1) {t1cUpInitial = He_t1c;}
				if (int(HeSpin)==0) {t1c0Initial = He_t1c;}
				if (int(HeSpin)==-1) {t1cDownInitial = He_t1c;}
				oldHeState = HeSpin;
				firstTime = false;
			}
*/
